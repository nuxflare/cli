...
async function sendMessage(message: string) {
  ...
  for await (const chunk of response) {
    ...
    if (chunk.relevantContext) {
      relevantContext.value.context = chunk.relevantContext.map(
...
const uploadedFiles = computed(() =>
  documents.value.filter((doc) => !doc.progress),
);
const isChatEnabled = computed(
  () => informativeMessage.value === "" && !!uploadedFiles.value.length,
);

const isExampleSession = useIsExampleSession();
const exampleSessions = useExampleSessions();
const exampleSession = computed(() =>
  exampleSessions.find((session) => session.id === sessionId.value),
);
</script>

<template>
  <div class="h-dvh flex flex-col md:flex-row max-h-dvh">
    <USlideover v-model:open="isDrawerOpen" :ui="{ content: 'md:hidden' }">
      <template #content>
    ...
    <div class="w-full h-full flex flex-col bg-zinc-50 dark:bg-zinc-950">
      ...
      <div class="overflow-y-auto h-full">
        <UContainer
          class="w-full h-full flex flex-col max-h-full max-w-3xl relative"
        >
          <ChatMessages
            v-show="!(isExampleSession && !messages.length)"
            :messages
          />

          <!-- Example questions -->
          <div
            v-if="isExampleSession && !messages.length"
            class="grid h-full p-3 place-items-center"
          >
            <div class="">
              <h2 class="font-medium text-lg">Try some sample questions</h2>
          ...
          <ChatInput
            class="w-full absolute bottom-0 inset-x-0"
