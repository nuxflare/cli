...
async function searchDocumentChunks(searchTerms: string[]) {
  const queries = searchTerms.filter(Boolean).map((term) => {
  ...
  const results = await Promise.all(
    queries.map(async (query) => {
      const { results } = (await useDrizzle().run(query)) as {
...
async function queryVectorIndex(queries: string[], sessionId: string) {
  const queryVectors = await Promise.all(
    queries.map((q) =>
      hubAI().run("@cf/baai/bge-large-en-v1.5", { text: [q] }),
    ),
  );
...
export async function processUserQuery(
  {
    sessionId,
    messages,
  }: { sessionId: string; messages: RoleScopedChatInput[] },
  ...
  const relevantDocs = await getRelevantDocuments(
    mergedResults.map((r) => r.id).slice(0, 10),
  );
